Ext.define('doweown.view.WorldCatView', {
	...
	itemTpl: '<div><tpl if="thumbnail"><img src="{thumbnail}"></tpl> <b>{title}</b><br> {author}<br> {date}<br> {publisher}<p> <br><b>Libraries:</b><br><tpl for="library"><i>{institutionName}</i><br> <a href="comgooglemaps://?q={streetAddress1}, {city},{state} {postalCode}" onclick="window.open(this.href,\'_system\'); return false;">map</a><p> <a href="{opacUrl}" onclick="window.open(this.href,\'_system\'); return false;">catalog link</a> <hr></tpl></div>'
     }
