Ext.define('doweown.view.WorldCatView', {
	...
	itemTpl: '<div><tpl if="thumbnail"><img src="{thumbnail}" style="float:left; padding: 5px 5px 4px 5px"></tpl> <b>{title}</b><br><i>{author}</i><br> {date}<br> {publisher}<br>{description}<p><b><a href="{borrowDirectUrl}" onclick="window.open(this.href,\'_system\'); return false;">Find it on Borrow Direct</a></b></div>'
     }
